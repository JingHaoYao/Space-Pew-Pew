...
public class FrontBasePlate: MonoBehaviour {
    ...
    public static bool frontRepair = false;
    public GameObject repairBot;
    public static bool frontPlateBroken = false;
    bool hasSet = false;
    ...
    void Update()
    {
        if (FrontPowerCore.powerCoreHealth > 0 || BackPowerCore.powerCoreHealth > 0)
        {
            ...
            transform.position = GameObject.Find("FourthBoss").transform.position + new Vector3(4.8f, 0, 0);
            if (numHit == 6)
            {
                boxCol.enabled = false;
                frontPlateBroken = true;
            }
            else
            {
                boxCol.enabled = true;
                frontPlateBroken = false;
            }
        }
        else
        {
