...
public class PirateCannon1 : MonoBehaviour {
    SpriteRenderer rend;
    Animator animator;
	...
	void Update(){
        ...
        else
        {
            ...
            if (isHit == false)
            {
                ...
                {
                    if (SecondBossScript.whichCannon == 1)
                    {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.gameObject.name != "BuccaneerShot(Clone)" && collision.gameObject.name != "CannonShot(Clone)" &&
           collision.gameObject.name != "RedPowderKeg(Clone)" && collision.gameObject.name != "BluePowderKeg(Clone)" &&
           collision.gameObject.name != "GreenPowderKeg(Clone)")
        {
            isHit = true;
            hitBox.enabled = false;
