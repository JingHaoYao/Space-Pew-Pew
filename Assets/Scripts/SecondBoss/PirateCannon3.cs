...
public class PirateCannon3 : MonoBehaviour
{
    SpriteRenderer rend;
    Animator animator;
    ...
    void Update()
    {
        ...
        {
            transform.position = GameObject.Find("PirateShipBaseBurning(Clone)").transform.position + new Vector3(0, -0.1f, 0);
        ...
        {
            transform.position = GameObject.Find("SecondBoss").transform.position + new Vector3(0f, -0.1f, 0);

            if (isHit == false)
            {
                if (SecondBossScript.fireCannon == true)
                {
                    if (SecondBossScript.whichCannon == 3)
                    {
